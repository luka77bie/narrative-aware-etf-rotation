...
SIGNAL_PATH = Path(
    "outputs/risk_adjusted_signal_history.csv"
)
...
def main() -> int:
    ...
    returns.to_csv(
        OUTPUT_DIRECTORY
        / "risk_adjusted_backtest_returns.csv"
    )

    holdings.to_csv(
        OUTPUT_DIRECTORY
        / "risk_adjusted_backtest_holdings.csv",
        index=False,
    )

    rebalances.to_csv(
        OUTPUT_DIRECTORY
        / "risk_adjusted_backtest_rebalances.csv",
        index=False,
    )

    metrics.to_csv(
        OUTPUT_DIRECTORY
        / "risk_adjusted_backtest_metrics.csv",
        index=False,
    )

    print("")
    print("Risk-Adjusted Backtest Configuration")
    print("=" * 60)
    print(f"Assets: {len(universe)}")
    print("Selection: Top 3 Equal Weight")
    print("Rebalance: Monthly")
    print("Signal: MOM60 - volatility and downside-risk penalties")
    ...
    print(
        "Returns: "
        "outputs/risk_adjusted_backtest_returns.csv"
    )
    print(
        "Holdings: "
        "outputs/risk_adjusted_backtest_holdings.csv"
    )
    print(
        "Rebalances: "
        "outputs/risk_adjusted_backtest_rebalances.csv"
    )
    print(
        "Metrics: "
        "outputs/risk_adjusted_backtest_metrics.csv"
    )
