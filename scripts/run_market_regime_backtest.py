...
def main() -> int:
    ...
    returns.to_csv(
        OUTPUT_DIRECTORY
        / "market_regime_backtest_returns.csv"
    )

    holdings.to_csv(
        OUTPUT_DIRECTORY
        / "market_regime_backtest_holdings.csv",
        index=False,
    )

    rebalances.to_csv(
        OUTPUT_DIRECTORY
        / "market_regime_backtest_rebalances.csv",
        index=False,
    )

    metrics.to_csv(
        OUTPUT_DIRECTORY
        / "market_regime_backtest_metrics.csv",
        index=False,
    )

    print("")
    print("MOM60 + Market Regime Backtest Configuration")
    print("=" * 60)
    ...
    print("Signal: MOM60 with CSI 300 market-regime filter")
    ...
    print(
        "Returns: "
        "outputs/market_regime_backtest_returns.csv"
    )
    print(
        "Holdings: "
        "outputs/market_regime_backtest_holdings.csv"
    )
    print(
        "Rebalances: "
        "outputs/market_regime_backtest_rebalances.csv"
    )
    print(
        "Metrics: "
        "outputs/market_regime_backtest_metrics.csv"
    )
